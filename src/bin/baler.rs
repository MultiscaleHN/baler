...
use baler::core::shell::{Shell, Verbosity};
use baler::util::{self, CliResult, lev_distance, Config, CargoResult, CargoError, CargoErrorKind};
use baler::util::CliError;
...
Some common baler commands are (see all commands with --list):
    ...
    new         Create a new baler project
    init        Create a new baler project in an existing directory
...
See 'baler help <command>' for more information on a specific command.
";
...
fn main() {
    ...
    let config = match Config::default() {
        ...
        Err(e) => {
            ...
            baler::exit_with_error(e.into(), &mut shell)
        }
    };
    ...
    let result = (|| {
        let args: Vec<_> = try!(env::args_os()
            ...
            .collect());
        let rest = &args;
        baler::call_main_without_stdin(execute, &config, USAGE, rest, true)
    })();

    match result {
        Err(e) => baler::exit_with_error(e, &mut *config.shell()),
        Ok(()) => {}
    }
}
...
/**
  The top-level `baler` command handles configuration and project location
...
fn execute(flags: Flags, config: &Config) -> CliResult {
    ...
    let _token = baler::util::job::setup();

    if flags.flag_version {
        let version = baler::version();
        println!("{}", version);
    ...
    let args = match &flags.arg_command[..] {
        // For the commands `baler` and `baler help`, re-execute ourselves as
        // `baler -h` so we can go through the normal process of printing the
        // help message.
        "" | "help" if flags.arg_args.is_empty() => {
            config.shell().set_verbosity(Verbosity::Verbose);
            let args = &["baler".to_string(), "-h".to_string()];
            return baler::call_main_without_stdin(execute, config, USAGE, args, false);
        }

        // For `baler help -h` and `baler help --help`, print out the help
        // message for `baler help`
        "help" if flags.arg_args[0] == "-h" || flags.arg_args[0] == "--help" => {
            vec!["baler".to_string(), "help".to_string(), "-h".to_string()]
        }

        // For `baler help foo`, print out the usage message for the specified
        // subcommand by executing the command with the `-h` flag.
        "help" => vec!["baler".to_string(), flags.arg_args[0].clone(), "-h".to_string()],

        // For all other invocations, we're of the form `baler foo args...`. We
...
fn try_execute_builtin_command(config: &Config, args: &[String]) -> Option<CliResult> {
    macro_rules! cmd {
        ($name:ident) => (if args[1] == stringify!($name).replace("_", "-") {
            config.shell().set_verbosity(Verbosity::Verbose);
            let r = baler::call_main_without_stdin($name::execute, config,
                                                   $name::USAGE,
                                                   &args,
                                                   false);
            return Some(r);
...
fn execute_external_subcommand(config: &Config, cmd: &str, args: &[String]) -> CliResult {
    let command_exe = format!("baler-{}{}", cmd, env::consts::EXE_SUFFIX);
    ...
    let baler_exe = config.baler_exe()?;
    let err = match util::process(&command)
        .env(baler::CARGO_ENV, baler_exe)
        .args(&args[1..])
        .exec_replace() {
        Ok(()) => return Ok(()),
        Err(e) => e,
...
fn init_git_transports(config: &Config) {
    ...
    match baler::ops::http_proxy_exists(config) {
        Ok(true) => {}
        _ => return,
    }

    let handle = match baler::ops::http_handle(config) {
        Ok(handle) => handle,
        Err(..) => return,
    };
