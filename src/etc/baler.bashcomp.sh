...
_baler()
{
	...
	if [[ $cword -eq 1 ]]; then
		if [[ "$cur" == -* ]]; then
			...
			COMPREPLY=( $( compgen -W "$__baler_commands" -- "$cur" ) )
		fi
...
complete -F _baler baler

__baler_commands=$(baler --list 2>/dev/null | tail -n +2)

_locate_manifest(){
	local manifest=`baler locate-project 2>/dev/null`
...
_get_targets(){
	...
	for FIND_PATH in ${FIND_PATHS[@]}; do
		if [[ -f "$FIND_PATH"/.baler/config ]]; then
		LINES=( `grep "$FIND_PATH"/.baler/config -e "^\[target\."` )
