...
pub fn install(root: Option<&str>,
    ...
    let overidden_target_dir = if source_id.is_path() {
        None
    } else if let Ok(td) = TempDir::new("baler-install") {
    ...
    let staging_dir = TempDir::new_in(&dst, "baler-install")?;
...
fn check_overwrites(dst: &Path,
    ...
    if !filter.is_specific() {
        ...
        // get checked during baler_compile, we only care about the "build
