...
pub fn init(opts: NewOptions, config: &Config) -> CargoResult<()> {
    ...
    if src_paths_types.len() == 0 {
        ...
        // Maybe when doing `baler init --bin` inside a library project stub,
    ...
    if version_control == None {
        ...
        // if none exists, maybe create git, like in `baler new`
    ...
    for i in &opts.source_files {
        if i.bin {
            if i.relative_path != "src/main.rs" {
                balertoml_path_specifier.push_str(&format!(r#"
[[bin]]
name = "{}"
path = {}
...
[dependencies]
{}"#, name, toml::Value::String(author), balertoml_path_specifier).as_bytes())?;
...
fn global_config(config: &Config) -> CargoResult<CargoNewConfig> {
    let name = config.get_string("baler-new.name")?.map(|s| s.val);
    let email = config.get_string("baler-new.email")?.map(|s| s.val);
    let vcs = config.get_string("baler-new.vcs")?;
