...
pub fn run(ws: &Workspace,
    ...
    let pkg = match options.spec {
        Packages::All => unreachable!("baler run supports single package only"),
        Packages::OptOut(_) => unreachable!("baler run supports single package only"),
        Packages::Packages(xs) => match xs.len() {
            ...
            _ => unreachable!("baler run supports single package only"),
        }
    };
    ...
    if bins.next().is_none() {
        if !options.filter.is_specific() {
            bail!("a bin target must be available for `baler run`")
        } else {
            // this will be verified in baler_compile
        }
    }
    if bins.next().is_some() {
        if !options.filter.is_specific() {
            bail!("`baler run` requires that a project only have one \
                   executable; use the `--bin` option to specify which one \
                   to run")
        } else {
            bail!("`baler run` can run at most one executable, but \
