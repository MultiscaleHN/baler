...
use balertest::support::{project, execs, git};
...
fn members_include_path_deps() {
    ...
    assert_that(p.baler("build").cwd(p.root().join("p1")),
                execs().with_status(0));
    assert_that(p.baler("build").cwd(p.root().join("p2")),
                execs().with_status(0));
    assert_that(p.baler("build").cwd(p.root().join("p3")),
                execs().with_status(0));
    assert_that(p.baler("build"),
                execs().with_status(0));
...
fn new_warns_you_this_will_not_work() {
    ...
    assert_that(p.baler("new").arg("--lib").arg("bar").env("USER", "foo"),
                execs().with_status(0)
                       .with_stderr("\
...
fn rebuild_please() {
    ...
    assert_that(p.baler("run").cwd(p.root().join("bin")),
                execs().with_status(0));
    ...
    assert_that(p.baler("build").cwd(p.root().join("lib")),
                execs().with_status(0));
...
fn lockfile_can_specify_nonexistant_members() {
    ...
    assert_that(p.baler("build").cwd(p.root().join("a")), execs().with_status(0));
}
...
fn workspace_with_transitive_dev_deps() {
    ...
    assert_that(p.baler("test").args(&["-p", "bar"]),
                execs().with_status(0));
}
...
fn relative_path_for_member_works() {
    ...
    assert_that(p.baler("build").cwd(p.root().join("foo")), execs().with_status(0));
    assert_that(p.baler("build").cwd(p.root().join("bar")), execs().with_status(0));
}
...
fn glob_syntax() {
    ...
    assert_that(p.baler("build").cwd(p.root().join("crates/bar")),
                execs().with_status(0));
    assert_that(&p.bin("foo"), existing_file());
    assert_that(&p.bin("bar"), existing_file());

    assert_that(p.baler("build").cwd(p.root().join("crates/baz")),
                execs().with_status(0));
    assert_that(&p.bin("foo"), existing_file());
    assert_that(&p.bin("baz"), existing_file());

    assert_that(p.baler("build").cwd(p.root().join("crates/qux")),
                execs().with_status(0));
