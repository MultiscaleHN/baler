...
"));
    ...
    assert_that(p.baler("build").arg("--features").arg("test"),
                execs().with_status(101).with_stderr("\
...
fn invalid6() {
    ...
    assert_that(p.baler_process("build").arg("--features").arg("foo"),
                execs().with_status(101).with_stderr("\
[ERROR] failed to parse manifest at `[..]`
...
", dir = p.url())));
    ...
    assert_that(p.baler("build").arg("--features").arg("bar"),
                execs().with_status(0).with_stderr(format!("\
...
fn many_cli_features() {
    ...
    assert_that(p.baler_process("build").arg("--features").arg("bar baz"),
                execs().with_status(0).with_stderr(format!("\
[COMPILING] ba[..] v0.0.1 ({dir}/ba[..])
[COMPILING] ba[..] v0.0.1 ({dir}/ba[..])
[COMPILING] foo v0.0.1 ({dir})
...
fn empty_features() {
    ...
    assert_that(p.baler_process("build").arg("--features").arg(""),
                execs().with_status(0));
}
...
fn no_rebuild_when_frobbing_default_feature() {
    ...
    assert_that(p.baler("build"), execs().with_status(0).with_stdout(""));
    assert_that(p.baler("build"), execs().with_status(0).with_stdout(""));
}
...
fn activating_feature_activates_dep() {
    ...
    assert_that(p.baler_process("build").arg("--features").arg("a").arg("-v"),
                execs().with_status(0));
}
...
fn dep_feature_in_cmd_line() {
    ...
    assert_that(p.baler("build").arg("--features").arg("derived/derived-feat"),
                execs().with_status(0));

    // Trying to enable features of transitive dependencies is an error
    assert_that(p.baler("build").arg("--features").arg("bar/some-feat"),
                execs().with_status(101).with_stderr("\
[ERROR] Package `foo v0.0.1 ([..])` does not have these features: `bar`
"));

    // Hierarchical feature specification should still be disallowed
    assert_that(p.baler("build").arg("--features").arg("derived/bar/some-feat"),
                execs().with_status(101).with_stderr("\
...
fn all_features_flag_enables_all_features() {
    ...
    assert_that(p.baler_process("build").arg("--all-features"),
                execs().with_status(0));
}
...
fn many_cli_features_comma_and_space_delimited() {
    ...
    assert_that(p.baler_process("build").arg("--features").arg("bar,baz bam bap"),
                execs().with_status(0).with_stderr(format!("\
[COMPILING] ba[..] v0.0.1 ({dir}/ba[..])
[COMPILING] ba[..] v0.0.1 ({dir}/ba[..])
[COMPILING] ba[..] v0.0.1 ({dir}/ba[..])
