...
use balertest::{sleep_ms, is_nightly};
use balertest::support::{project, execs, basic_bin_manifest, basic_lib_manifest, baler_exe};
...
fn baler_test_release() {
    ...
    assert_that(p.baler_process("test").arg("-v").arg("--release"),
                execs().with_status(0).with_stderr(format!("\
...
fn baler_test_verbose() {
    ...
    assert_that(p.baler_process("test").arg("-v").arg("hello"),
                execs().with_status(0).with_stderr(format!("\
...
fn many_similar_names() {
    ...
    let output = p.baler_process("test").arg("-v").exec_with_output().unwrap();
...
fn dont_run_examples() {
    let p = project("foo")
        ...
        .file("examples/dont-run-me-i-will-fail.rs", r#"
            fn main() { panic!("Examples should not be run by 'baler test'"); }
        "#);
    assert_that(p.baler_process("test"),
                execs().with_status(0));
}
...
fn pass_through_command_line() {
    ...
    assert_that(p.baler_process("test").arg("bar"),
                execs().with_status(0)
                       .with_stderr(&format!("\
...
[DOCTEST] foo", dir = p.url()))
    ...
    assert_that(p.baler("test").arg("foo"),
                execs().with_status(0)
                       .with_stderr("\
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
[RUNNING] target[/]debug[/]deps[/]foo-[..][EXE]
...
// Regression test for running baler-test twice with
// tests in an rlib
#[test]
fn baler_test_twice() {
...
fn test_no_run() {
    ...
    assert_that(p.baler_process("test").arg("--no-run"),
                execs().with_status(0)
                       .with_stderr(&format!("\
[COMPILING] foo v0.0.1 ({dir})
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
...
fn test_run_specific_bin_target() {
    ...
    assert_that(prj.baler_process("test").arg("--bin").arg("bin2"),
                execs().with_status(0)
                       .with_stderr(format!("\
[COMPILING] foo v0.0.1 ({dir})
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
...
fn test_run_implicit_bin_target() {
    ...
    assert_that(prj.baler_process("test").arg("--bins"),
                execs().with_status(0)
                       .with_stderr(format!("\
[COMPILING] foo v0.0.1 ({dir})
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
...
fn test_run_specific_test_target() {
    ...
    assert_that(prj.baler_process("test").arg("--test").arg("b"),
                execs().with_status(0)
                       .with_stderr(format!("\
[COMPILING] foo v0.0.1 ({dir})
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
...
fn test_run_implicit_test_target() {
    ...
    assert_that(prj.baler_process("test").arg("--tests"),
                execs().with_status(0)
                       .with_stderr(format!("\
[COMPILING] foo v0.0.1 ({dir})
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
...
fn test_run_implicit_bench_target() {
    ...
    assert_that(prj.baler_process("test").arg("--benches"),
                execs().with_status(0)
                       .with_stderr(format!("\
[COMPILING] foo v0.0.1 ({dir})
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
...
fn test_run_implicit_example_target() {
    ...
    assert_that(prj.baler_process("test").arg("--examples"),
                execs().with_status(0)
                       .with_stderr(format!("\
[COMPILING] foo v0.0.1 ({dir})
...
fn test_no_harness() {
    ...
    assert_that(p.baler_process("test").arg("--").arg("--nocapture"),
                execs().with_status(0)
                       .with_stderr(&format!("\
[COMPILING] foo v0.0.1 ({dir})
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
...
fn selective_testing() {
    ...
    println!("d1");
    assert_that(p.baler("test").arg("-p").arg("d1"),
                execs().with_status(0)
                       .with_stderr(&format!("\
...
[RUNNING] target[/]debug[/]deps[/]d1-[..][EXE]", dir = p.url()))
    ...
    println!("d2");
    assert_that(p.baler("test").arg("-p").arg("d2"),
                execs().with_status(0)
                       .with_stderr(&format!("\
...
fn build_then_selective_test() {
    ...
    assert_that(p.baler_process("build"), execs().with_status(0));
    p.root().move_into_the_past();
    assert_that(p.baler("test").arg("-p").arg("b"),
                execs().with_status(0));
}
...
fn example_bin_same_name() {
    ...
    assert_that(p.baler_process("test").arg("--no-run").arg("-v"),
                execs().with_status(0)
                       .with_stderr(&format!("\
[COMPILING] foo v0.0.1 ({dir})
[RUNNING] `rustc [..]`
...
fn bad_example() {
    ...
    assert_that(p.baler("run").arg("--example").arg("foo"),
                execs().with_status(101).with_stderr("\
[ERROR] no example target named `foo`
"));
    assert_that(p.baler("run").arg("--bin").arg("foo"),
                execs().with_status(101).with_stderr("\
...
fn doctest_feature() {
    ...
    assert_that(p.baler_process("test").arg("--features").arg("bar"),
                execs().with_status(0)
                       .with_stderr("\
[COMPILING] foo [..]
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
...
fn filter_no_doc_tests() {
    ...
    assert_that(p.baler_process("test").arg("--test=foo"),
...
fn no_fail_fast() {
    let p = project("foo")
        ...
        "#);
    assert_that(p.baler_process("test").arg("--no-fail-fast"),
                execs().with_status(101)
...
fn test_multiple_packages() {
    ...
    assert_that(p.baler("test").arg("-p").arg("d1").arg("-p").arg("d2"),
                execs().with_status(0)
                       .with_stderr_contains("\
[RUNNING] target[/]debug[/]deps[/]d1-[..][EXE]")
                       .with_stderr_contains("\
...
fn bin_does_not_rebuild_tests() {
    ...
    assert_that(p.baler("test").arg("-v").arg("--no-run"),
                execs().with_status(0)
                       .with_stderr("\
[COMPILING] foo v0.0.1 ([..])
[RUNNING] `rustc [..] src[/]main.rs [..]`
...
fn selective_test_wonky_profile() {
    ...
    assert_that(p.baler("test").arg("-v").arg("--no-run").arg("--release")
...
fn only_test_docs() {
    ...
    assert_that(p.baler("test").arg("--doc"),
                execs().with_status(0)
                       .with_stderr("\
[COMPILING] foo v0.0.1 ([..])
[FINISHED] dev [unoptimized + debuginfo] target(s) in [..]
...
fn test_release_ignore_panic() {
    ...
    p.build();
    println!("test");
    assert_that(p.baler("test").arg("-v"), execs().with_status(0));
    println!("bench");
    assert_that(p.baler("bench").arg("-v"), execs().with_status(0));
}
...
fn doctest_only_with_dev_dep() {
    ...
    assert_that(p.baler_process("test").arg("--doc").arg("-v"),
                execs().with_status(0));
}
...
fn test_many_targets() {
    ...
    assert_that(p.baler_process("test").arg("--verbose")
...
fn doctest_and_registry() {
    ...
    assert_that(p.baler_process("test").arg("--all").arg("-v"),
                execs().with_status(0));
}
...
fn test_order() {
   let p = project("foo")
        ...
        assert_that(p.baler_process("test").arg("--all"),
            execs().with_status(0)
                   .with_stdout_contains("
running 1 test
test test_lib ... ok
